...
def singleSampleCompass(
    ...
    reaction_penalties = pd.read_csv(
        os.path.join(args["penalties_dir"], f"sample{sample_index}", "penalties.txt.gz"), index_col=0, sep="\t"
    ).iloc[:, 0]
    ...
    if not args["no_reactions"]:
        ...
        reaction_scores = compass_reactions(
            model,
            opt=opt,
            reaction_penalties=reaction_penalties_dict,
            perf_log=perf_log,
            args=args,
            preprocess_cache_dir=preprocess_cache_dir,
        )
    ...
    if not args["no_reactions"]:
        ...
        reaction_scores.to_csv(os.path.join(directory, "reactions.txt"), sep="\t", header=True)
    ...
    if args["calc_metabolites"]:
        ...
        uptake_scores.to_csv(os.path.join(directory, "uptake.txt"), sep="\t", header=True)
        secretion_scores.to_csv(os.path.join(directory, "secretions.txt"), sep="\t", header=True)

    if args["generate_cache"] or cache.is_new_cache(model):
        logger.info("Saving cache file for Model: {}, Media: {}".format(model.name, model.media))
...
def compass_exchange(
    ...
    for metabolite in tqdm(metabolites, file=sys.stderr):
        ...
        if args["select_reactions"] or args["select_subsystems"]:
            ...
            reactions = [
                r for r in reactions if ((r.id)[:-4] in selected_reaction_ids or str(r.id) in selected_reaction_ids)
            ]
...
def compass_reactions(
    ...
    if args["select_reactions"] or args["select_subsystems"]:
        ...
        reactions = [
            r for r in reactions if (str(r.id)[:-4] in selected_reaction_ids or str(r.id) in selected_reaction_ids)
        ]
    ...
    for reaction in tqdm(reactions, file=sys.stderr):
        ...
        if r_max == 0:
            ...
            # Minimize Penalty
            delta = LinearProgramDelta(
                objective=reaction_penalties, sense="min", blocked_reactions=blocked_reactions, high_flux=high_flux
            )
...
def maximize_reaction_range(start_stop, args, model_name=None, metabolic_model_dir=MODEL_DIR):
    ...
    for reaction in tqdm(reactions, file=sys.stderr):
        ...
        delta = LinearProgramDelta(
            objective=objective, sense="max", blocked_reactions=blocked_reactions, added_secretion={}, added_uptake={}
        )
        sol = solve_model_wrapper(opt, delta)
